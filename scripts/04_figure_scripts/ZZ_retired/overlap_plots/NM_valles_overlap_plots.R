...
# Import data
canopy_data_raw <- read_csv("data/01_primary/temperate/NM_valles_caldera/derivative/NM_valles_caldera_canopy_wide.csv")
soil_data_raw <- read_csv("data/01_primary/temperate/NM_valles_caldera/derivative/NM_valles_caldera_soil_wide.csv")

data_raw <- canopy_data_raw %>% 
  mutate(site = recode(site, "NM_canopy" = "NM_soil")) %>% 
  full_join(soil_data_raw)

# Designate site name
SITE_NAME <- "Valles Caldera Mountains (coniferous forests)"

# Designate elevations of low and high sites
LOW_ELEVATION <- "2500"
HIGH_ELEVATION <- "3003"
# Set file path where figure will be saved
OUTPUT_FILE_PATH <- "figures/NM_valles_2011.png"
...
# For the Sierra gradient, we need to remove the high elevation site because
#   it has no soil or ground data...and so the na.omit will remove everything
data <- data %>%
  select(-contains("mid"), -contains("ground"), -contains("surface"))
...
# Create figure axes
plot(data$julian, data$low_canopy_max, axes=FALSE, type='n', bty="n", 
...
# lowlands
# Plot lines for lowlands min and max temp
lines(data$julian, data$low_canopy_min, col = 'grey')
lines(data$julian, data$low_canopy_max, col = 'grey')

# Create a polygon to fill in between the lines
polygon(c(data$julian, rev(data$julian)),
        c(data$low_canopy_max, rev(data$low_canopy_min)),
        col = adjustcolor("red", alpha=.5), border = NA)


# Uplands
# Plot lines for uplands min and max temp
lines(data$julian, data$high_canopy_min, col = 'grey')
lines(data$julian, data$high_canopy_max, col = 'grey')

# Create a polygon to fill in between the lines
polygon(c(data$julian, rev(data$julian)),
        c(data$high_canopy_max, rev(data$high_canopy_min)),
...
mtext("canopy temp", NORTH<-3, cex=1.5,col="black")
