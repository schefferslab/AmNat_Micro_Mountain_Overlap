...
# Import data
data_raw <- read_csv("./data/01_primary/temperate/CA_sierra/derivative/low_to_high/sierra_wide.csv")

# Designate site name
SITE_NAME <- "Sierra Mountains (mixed coniferous forests)"

# Designate elevations of low and mid sites
LOW_ELEVATION <- "1160"
MID_ELEVATION <- "2710"
# Set file path where figure will be saved
OUTPUT_FILE_PATH <- "./figures/CA_sierra_low_to_high_2017.png"
...
legend("bottomleft", c(paste0("lowlands (", LOW_ELEVATION, " m asl)"), 
                       paste0("Uplands (", MID_ELEVATION, " m asl)")), 
...
lines(data$julian, data$mid_soil_min, col = 'grey')
lines(data$julian, data$mid_soil_max, col = 'grey')

#create filled polygon in between the lines
polygon(c(data$julian, rev(data$julian)),
        c(data$mid_soil_max, rev(data$mid_soil_min)),
...
# Create figure axes
plot(data$julian, data$low_ground_max, type='n', 
     bty="n", cex.axis=1.5, ylim=c(-20, 50), xlim=c(0,365), ylab='temperature (C)', 
     xlab='Julian Day')
...
# lowlands
# Plot lines for lowlands min and max temp
lines(data$julian, data$low_ground_min, col = 'grey')
lines(data$julian, data$low_ground_max, col = 'grey')

# Create a polygon to fill in between the lines
polygon(c(data$julian, rev(data$julian)), 
        c(data$low_ground_max, rev(data$low_ground_min)), 
        col = adjustcolor("light green", alpha=.5), border = NA)

# Uplands
# Plot lines for uplands min and max temp
lines(data$julian, data$mid_ground_min, col = 'grey')
lines(data$julian, data$mid_ground_max, col = 'grey')

#create filled polygon in between the lines
polygon(c(data$julian, rev(data$julian)),
        c(data$mid_ground_max, rev(data$mid_ground_min)),
...
# Uplands
# Plot lines for uplands min and max temp
lines(data$julian, data$mid_air_min, col = 'grey')
lines(data$julian, data$mid_air_max, col = 'grey')

# Create a polygon to fill in between the lines
polygon(c(data$julian, rev(data$julian)),
        c(data$mid_air_max, rev(data$mid_air_min)),
...
mtext("Air", NORTH<-3, cex=1.5,col="black")
