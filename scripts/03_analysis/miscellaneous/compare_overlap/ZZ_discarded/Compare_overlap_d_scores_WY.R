...
#only conducted for temperate data
#dependant on objects created in "North Carolina Overlap Script_DHK edits"

## Prep workspace #############
library(tidyverse)

sage_raw <- read.csv("./data/02_derivative/wyoming_all_elevations.csv")

sage <- sage_raw %>%
  mutate(overlap_v1 = min_soil_temp

soil_overlap <- ((play_1700_soil$Temp_max-play_500_soil$Temp_min))/(sqrt(play_1700_soil$Temp_max-play_1700_soil$Temp_min)*(play_500_soil$Temp_max-play_500_soil$Temp_min))
play_500_soil$overlap <-soil_overlap
overlap_soil_500 <- data.frame(play_500_soil$julian_max, play_500_soil$overlap)

#second overlap value

soil_overlap_v2 <- ((play_1700_soil$Temp_max-play_500_soil$Temp_min))
