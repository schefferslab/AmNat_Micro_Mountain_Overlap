...
soil_raw <- read_csv("./data/01_primary/temperate/OR_andrews/derivative/OR_andrewsRS89RS04_soil_wide.csv")
surface_raw <- read_csv("data/01_primary/temperate/OR_andrews/derivative/OR_andrewsFineRes_wide.csv")
...
YEAR <- 2003
JULIAN <- 10
PLOT_TITLE <- "Oregon: Andrews Exp Forest (deciduous)"
...
three_monthly <- soil_raw %>%
  mutate(month = month(high_DATE_TIME)) %>% 
...
## ....B. Calc overlap for increasing temporal intervals #########
surface_raw <- soil_raw %>% 
  select(year, julian, high_DATE_TIME) %>% 
  right_join(surface_raw)

three_monthly <- surface_raw %>%
  mutate(month = month(high_DATE_TIME)) %>% 
  mutate(interval = cut(month,
                        breaks=c(0, 3, 6, 9, 12), 
                        labels=c("1","2","3","4")))

six_monthly <- surface_raw %>%
  mutate(month = month(high_DATE_TIME)) %>% 
  mutate(interval = cut(month,
                        breaks=c(0, 6, 12), 
                        labels=c("1","2")))
