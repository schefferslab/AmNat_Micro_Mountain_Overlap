...
mountains_janz_month <- read_csv("data/03_compiled/mountains_janz_daily_avg.csv",
                                 col_types = cols(
                                   year = col_double(),
                                   elevation_change = col_double()
                                 ))
dscore_data <- mountains_janz_month
...
## ....2f. Control for/plot elevation difference ########

dscore_data <- dscore_data %>% 
  mutate(overlap = overlap * elevation_change / 1000) %>% 
  mutate(overlap_v2 = overlap_v2 * elevation_change / 1000)
...
soil <- ggplot(data = filter(dscore_data, micro == "soil"), 
  ...
  #geom_point(aes(shape = factor(site), color = factor(site)), size = 3) +
  coord_cartesian(xlim = NULL, ylim = c(-0.003, 0.003), expand = FALSE) +
  theme_bw(base_size = 14) +
...
soil <- ggplot(data = filter(dscore_data, micro == "soil"), 
               aes(x = site, y = overlap_v2, fill = seasonal_attributes)) +
  geom_boxplot(aes(x = site, y = overlap_v2), alpha = 0.5, lwd = .5) +
...
soil <- ggplot(data = filter(dscore_data, micro == "soil"), aes(x = macro, y = overlap_v2, fill = factor(macro))) +
  scale_fill_manual(values = c("#307233", "#b0db39", "#6cba3b", "#d3c250")) +
  geom_boxplot(aes(x = macro, y = overlap_v2, fill = macro), alpha = 0.7, lwd = .5) +
  # scale_color_viridis(discrete = TRUE, option = "C") +
  #geom_point(aes(shape = factor(site), color = factor(site)), size = 3) +
  coord_cartesian(xlim = NULL, ylim = c(-2.5, 20), expand = FALSE) +
  theme_bw(base_size = 14) +
  theme(legend.position = "none") +
  labs(x = "Soil")

surface <- ggplot(data = filter(dscore_data, micro == "surface"), 
                  aes(x = macro, y = overlap_v2, fill = factor(macro))) +
  scale_fill_manual(values = c("#307233", "#b0db39", "#6cba3b", "#d3c250")) +
  geom_boxplot(aes(x = macro, y = overlap_v2, fill = macro), alpha = 0.7, lwd = .5) +
  scale_color_viridis(discrete = TRUE, option = "C") +
  #geom_point(aes(shape = factor(site), color = factor(site)), size = 3) +
...
canopy <- ggplot(data = filter(dscore_data, micro == "canopy"), aes(x = macro, y = overlap_v2, fill = factor(macro))) +
  scale_fill_manual(values = c("#307233", "#3e9de0", "#d3c250", "#b0db39", "#812dc6"), guide = guide_legend(title = "macro")) +
  geom_boxplot(aes(x = macro, y = overlap_v2, fill = macro), alpha = 0.7, lwd = .5) +
  scale_color_viridis(discrete = TRUE, option = "C", guide = FALSE) +
  #geom_point(aes(shape = factor(site), color = factor(site)), size = 3) +
...
## ....3G. Boxplot plots, grouped by macro, micro, and foliage/snow ###########
...
soil <- ggplot(data = filter(dscore_data, micro == "soil"), 
               aes(x = seasonal_attributes, y = overlap, fill = factor(macro))) +
  geom_boxplot(aes(x = macro, y = overlap, fill = seasonal_attributes), alpha = 0.7, lwd = .5) +
  scale_fill_manual(values = c("#3ee0ac", "#3e9de0", "#3e9de0",
                               "#307233", "#e08626", "#e5d35e")) +
  coord_cartesian(xlim = NULL, ylim = c(-0.003, 0.003), expand = FALSE) +
  theme_bw(base_size = 14) +
...
surface <- ggplot(data = filter(dscore_data, micro == "surface"), 
                  aes(x = seasonal_attributes, y = overlap, fill = factor(macro))) +
  geom_boxplot(aes(x = macro, y = overlap, fill = seasonal_attributes), 
               alpha = 0.7, lwd = .5) +
  scale_fill_manual(values = c("#3ee0ac", "#3e9de0", "#307233", 
                               "#e08626", "#e5d35e")) +
  coord_cartesian(xlim = NULL, ylim = c(-0.003, 0.003), expand = FALSE) +
...
## ....3H. Plot overlap by elevation difference ########
...
## ....3J. Boxplot with overlayed mean and standard error...but no jitter ###########
ggplot(dscore_data, aes(x = micro, y = overlap, fill = factor(macro))) +
