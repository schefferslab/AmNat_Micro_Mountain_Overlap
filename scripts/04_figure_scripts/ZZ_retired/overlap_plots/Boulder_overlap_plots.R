...
# Import data
data_raw <- read_csv("data/01_primary/temperate/CO_boulder/derivative/Boulder_met1-GLV_wide.csv")

# Designate site name
SITE_NAME <- "Boulder mountains (open surface)"

# Designate elevations of low and high sites
LOW_ELEVATION <- "1948"
HIGH_ELEVATION <- "3800"
# Set file path where figure will be saved
OUTPUT_FILE_PATH <- "./figures/overlap_figures/single_year_only/Boulder/Boulder_2012.png"
...
plot(data$julian, data$low_soil_max, type='n', 
     bty="n", cex.axis=1.5, ylim=c(-40, 50), xlim=c(0,365), ylab='temperature (C)', 
     xlab='Julian Day')
...
# Create figure axes
plot(data$julian, data$low_surface_max, axes=FALSE, type='n', bty="n", 
     cex.axis=1.5,ylim=c(-40,50), xlim=c(0,365), ylab="", xlab='Julian Day')
