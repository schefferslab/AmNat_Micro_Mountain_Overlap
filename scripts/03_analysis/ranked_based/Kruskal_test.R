...
# D-scores
kruskal.test(janzenDscore ~ macro, data = mountains_janz)

# TAD
kruskal.test(TAD ~ macro, data = mountains_janz)


## 3. Compare minmax regimes between micros ########

mountains_minmax <- mountains %>%
  gather(key = "minmax", "temp", min, max)


kruskal.test(temp ~ macro, data = mountains_minmax)

kruskal.test(temp ~ micro, data = mountains_minmax)
...
kruskal.test(janzenDscore ~ micro, data = mountains_janz)
kruskal.test(TAD ~ micro, data = mountains_janz)

# minmax
# for all sites combined
# for each site separately
kruskal.test(temp ~ micro, data = mountains_janz)
