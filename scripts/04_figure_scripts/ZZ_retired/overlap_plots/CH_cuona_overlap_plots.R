...
# Import data
data_raw <- read_csv("data/01_primary/temperate/CH_cuona/derivative/CH_cuona_wide.csv")

# Designate site name
SITE_NAME <- "Himilayas Cuona"

# Designate elevations of low and high sites
LOW_ELEVATION <- "3200"
HIGH_ELEVATION <- "4700"
# Set file path where figure will be saved
OUTPUT_FILE_PATH <- "./figures/overlap_figures/CH_cuona_2019.png"
...
data <- data %>% 
  filter(complete.cases(low_surface_max), complete.cases(low_surface_min),
         complete.cases(high_surface_max), complete.cases(high_surface_min))
