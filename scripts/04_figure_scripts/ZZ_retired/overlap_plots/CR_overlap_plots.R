...
# Import data
data_raw <- read_csv("data/01_primary/tropical/Costa_Rica/derivative/CR_wide.csv")

# Designate site name
SITE_NAME <- "Costa Rica Basham sites"

# Designate elevations of low and mid sites
LOW_ELEVATION <- "78"
HIGH_ELEVATION <- "2064"
# Set file path where figure will be saved
OUTPUT_FILE_PATH <- "figures/overlap_figures/single_year_only/CR_Basham/CR_2017.png"
...
# Select down cols to filter out NAs
data <- data %>% 
  select(julian, year, contains("low"), contains("high")) %>% 
...
plot(data$julian, data$low_soil_max, type='n', 
     bty="n", cex.axis=1.5, ylim=c(-20, 50), xlim=c(142,210), ylab='temperature (C)', 
     xlab='Julian Day')
...
# Create figure axes
plot(data$julian, data$low_surface_max, type='n', 
     bty="n", cex.axis=1.5, ylim=c(-20, 50), xlim=c(142,210), ylab='temperature (C)', 
     xlab='Julian Day')
...
plot(data$julian, data$low_canopy_max, axes=FALSE, type='n', bty="n", 
     cex.axis=1.5,ylim=c(-20,50), xlim=c(142,210), ylab="", xlab='Julian Day')
