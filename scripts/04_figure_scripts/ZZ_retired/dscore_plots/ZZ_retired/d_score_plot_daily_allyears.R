...
mountains_janz_daily <- read_csv("data/03_compiled/mountains_janz_daily.csv",
                                 col_types = cols(
                                   year = col_double(),
                                   elevation_change = col_double()
                                 ))
dscore_data <- mountains_janz_daily
...
ggplot(filter(dscore_data_outOmit, micro == "soil",
              macro == "leaf-off" | macro == "non-forest"),
       aes((overlap))) +
